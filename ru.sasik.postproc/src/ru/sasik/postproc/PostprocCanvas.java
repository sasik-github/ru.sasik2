...
public class PostprocCanvas extends JPanel {
	...
	private static final Integer CONVERTER_DX = 100;

	private static final Integer CONVERTER_DY = -100;

	private static final Integer DX = 200;

	private static final Integer DY = 200;
	...
	protected void paintComponent(Graphics g) {
		...
		for (ArrayList<Double> node : currentZone.getAll()) {
			g.drawOval(
					(int)((node.get(0) * CONVERTER_DX)  + DX), (int)((node.get(1) * CONVERTER_DY) + DY), 5, 5);
		}
